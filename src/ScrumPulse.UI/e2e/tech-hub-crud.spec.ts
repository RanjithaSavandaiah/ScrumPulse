...
test.describe('Tech Hub: Tech Debt Backlog & Tech Talks Lifecycle', () => {
  ...
  test('should log, resolve, and delete technical debt item', async ({ page }) => {
    ...
    // 4. Delete Tech Debt via confirm modal
    const deleteBtn = debtCard.locator('.btn-delete');
    ...
    // Verify removed
    await expect(page.locator('.hub-card', { hasText: debtTitle })).not.toBeVisible({ timeout: 10000 });
  });

  test('should schedule, view, and delete a tech sharing session', async ({ page }) => {
    const timestamp = Date.now();
    ...
    // 4. Delete Tech Talk via confirm modal
    const deleteBtn = talkCard.locator('.btn-delete');
    await expect(deleteBtn).toBeVisible();
    await deleteBtn.click();
    ...
    // Verify removed
    await expect(page.locator('.talk-card', { hasText: talkTopic })).not.toBeVisible({ timeout: 10000 });
  });
});
